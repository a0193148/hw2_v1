...
void *recvsock(void *arg)
{
	...
	while(1){
		...
		if(strcmp(data, "quit")==0) break;
			...
			printf("\n\t----- Illegal player name ! Please try again ! -----\n\n");
		}
		else if(strcmp(data, "Error")==0){
...
void *sendsock(void *arg)
{
	...
	printf("*\t\t\tGame Menu\t\t\t\t*\n");
	printf("*\t<menu> --- Show the command\t\t\t\t*\n");
	printf("*\t<list> --- Show players who are online\t\t\t*\n");
	...
	while(1){
		...
		if(strcmp(command, "quit")==0){
			...
			printf("\n*\t\t\tGame Menu\t\t\t*\n");
			printf("*\t<menu> --- Show the command\t\t\t*\n");
			printf("*\t<list> --- Show the players who are online\t*\n");
			printf("*\t<match> --- Create the new room\t\t\t*\n");
			printf("*\t<quit> --- Exit the game\t\t\t*\n\n");
		}
	...
	while(1){
		...
		if(strcmp(data, "quit")==0 || strcmp(data, "Win")==0 || strcmp(data, "Lose")==0 || strncmp(data, "Even", 4)==0 || strncmp(data, "Leave", 5)==0){
			...
			while(1){
				...
				else printf("----- Illegal input !! Please enter the correct number(from  1 to 9) !!\n\n");
			}
